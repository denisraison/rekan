  ...
  {#if ideaDrafts !== null}
    <div class="hidden md:flex flex-col gap-3 mb-2">
      <div class="flex items-center justify-between">
        ...
        <Button onclick={ondismissideas} variant="ghost" size="sm" class="text-muted-foreground py-1 min-h-0">Cancelar</Button>
      </div>
  ...
  {#if !blockReason}
      ...
      {#if inputMode === 'generate' && ideaDrafts === null}
        ...
        {#if showGenerateIdeasButton}
          <Button
            ...
            variant="secondary"
            size="sm"
            class="gap-1.5 text-text-secondary"
          >
            {#if generatingIdeas}
    ...
    <div class="flex gap-2 items-center relative">
      <Button
        ...
        variant="ghost"
        size="icon"
        class="shrink-0 w-10 h-10 text-muted-foreground"
      ...
      {#if showAttachMenu}
        ...
        <div class="absolute bottom-12 left-0 z-20 rounded-xl shadow-lg border border-[--border-strong] p-2 flex flex-col gap-1 bg-[--bg] min-w-[180px]">
          <Button
            ...
            variant="ghost"
            size="sm"
            class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-foreground justify-start"
          >
      ...
      {#if inputMode === 'generate'}
        <Button
          ...
          size="sm"
          class="shrink-0 px-3 md:px-5 py-3 gap-2"
        >
          {#if generating}
      ...
      {:else}
        <Button
          ...
          variant="whatsapp"
          size="sm"
          class="shrink-0 px-3 md:px-5 py-3"
        >
          {sendingQuick ? "..." : "Enviar"}
        </Button>
      {/if}

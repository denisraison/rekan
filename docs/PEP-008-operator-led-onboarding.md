...
4. Show Terms & Conditions (inline expandable section). Content covers: provider identification (razão social, CNPJ, endereço), service description (AI-assisted content with human review), dynamic price per tier/commitment, explicit Pix Automático recurring debit authorization, launch price disclaimer, 30-day money-back guarantee, CDC Art. 49 right of withdrawal (7 days), cancellation policy (mensal: immediate, trimestral: service until end of paid period), LGPD compliance (legal basis Art. 7 V, data controller, retention period, data subject rights), content ownership, foro. See `web/src/routes/convite/[token]/+page.svelte` for the full text.

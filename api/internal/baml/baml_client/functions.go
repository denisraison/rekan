...
func AgentProcess(ctx context.Context, operatorName string, message string, systemContext string, conversationHistory string, opts ...CallOptionFunc) (types.AgentResponse, error) {
	...
	if callOpts.onTick == nil {
		result, err := bamlRuntime.CallFunction(ctx, "AgentProcess", encoded, callOpts.onTick)
		if err != nil {
		...
		channel, err := bamlRuntime.CallFunctionStream(ctx, "AgentProcess", encoded, callOpts.onTick)
		if err != nil {
			return types.AgentResponse{}, err
		}
...
func ExtractProfileSignal(ctx context.Context, message string, businessType string, opts ...CallOptionFunc) (*types.ProfileSignal, error) {
	...
	if callOpts.onTick == nil {
		result, err := bamlRuntime.CallFunction(ctx, "ExtractProfileSignal", encoded, callOpts.onTick)
		if err != nil {
			return nil, err
		}
		...
		casted := (result.Data).(*types.ProfileSignal)

		return casted, nil
	} else {
		channel, err := bamlRuntime.CallFunctionStream(ctx, "ExtractProfileSignal", encoded, callOpts.onTick)
		if err != nil {
			return nil, err
		}
		...
		for result := range channel {
			...
			if result.HasData {
				return result.Data.(*types.ProfileSignal), nil
			}
		}

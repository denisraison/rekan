<script lang="ts">
	...
	async function submitStep2() {
		const valid = services.every((s: Service) => s.name.trim() && s.price_brl > 0);
	...
	async function submitStep3(skip = false) {
		...
		try {
			await pb.collection('businesses').update(businessId, {
				target_audience: skip ? 'Público geral' : (target_audience.trim() || 'Público geral'),
				brand_vibe: skip ? 'Profissional e acolhedor' : (brand_vibe.trim() || 'Profissional e acolhedor'),
	...
	<div class="max-w-lg mx-auto">
		...
		<div class="flex items-center gap-2 mb-8">
			{#each [1, 2, 3] as s}
				<div
					...
					style="background: {s <= step ? 'var(--primary)' : 'var(--border-strong)'}"
				></div>
			{/each}
		</div>

		{#if step === 1}
			<div class="rounded-2xl p-8" style="background: var(--surface); box-shadow: var(--shadow-md); border: 1px solid var(--border)">
				<h1 class="text-xl font-semibold mb-1" style="color: var(--text); font-family: var(--font-primary)">
					Sobre o seu negócio
				</h1>
				<p class="text-sm mb-6" style="color: var(--text-secondary)">Etapa 1 de 3</p>

				{#if error}
					<p class="text-sm mb-4 p-3 rounded-lg" style="color: #DC2626; background: #FEF2F2">{error}</p>
				{/if}
		...
		{:else if step === 2}
			<div class="rounded-2xl p-8" style="background: var(--surface); box-shadow: var(--shadow-md); border: 1px solid var(--border)">
				...
				<p class="text-sm mb-6" style="color: var(--text-secondary)">Etapa 2 de 3</p>
		...
		{:else if step === 3}
			<div class="rounded-2xl p-8" style="background: var(--surface); box-shadow: var(--shadow-md); border: 1px solid var(--border)">
				...
				<div class="flex flex-col gap-4">
					<label class="flex flex-col gap-1.5">
						<span class="text-sm font-medium" style="color: var(--text)">Público-alvo</span>
						<input
					...
					<label class="flex flex-col gap-1.5">
						<span class="text-sm font-medium" style="color: var(--text)">Estilo da marca</span>
						<input
					...
					<label class="flex flex-col gap-1.5">
						<span class="text-sm font-medium" style="color: var(--text)">
							Diferenciais
							<span class="font-normal" style="color: var(--text-muted)">— opcional</span>
						</span>
						<textarea
							bind:value={quirks}

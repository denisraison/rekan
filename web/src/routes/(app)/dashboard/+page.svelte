<script lang="ts">
	...
	let editStates = $state<Record<string, { caption: string; hashtags: string }>>({});

	let batches = $derived(groupByBatch(posts));
	let generationsLeft = $derived(
		user?.subscription_status === 'trial' ? trialLimit - (user?.generations_used ?? 0) : null
	);

	function groupByBatch(all: Post[]): Array<{ batchId: string; date: string; posts: Post[] }> {
	...
	async function generate() {
		...
		try {
			await pb.send(`/api/businesses/${business.id}/posts:generate`, { method: 'POST' });
			await pb.collection('users').authRefresh();
			user = pb.authStore.model as unknown as User;
			await loadPosts();
		} catch (err: unknown) {
	...
	async function subscribe() {
		...
		try {
			...
			generateError = e?.data?.message ?? 'Erro ao iniciar assinatura. Tente novamente.';
		} finally {
			subscribing = false;
		}
	}
	...
	function startEdit(post: Post) {
		editStates[post.id] = { caption: post.caption, hashtags: post.hashtags.join(' ') };
	}
	...
	async function saveEdit(post: Post) {
		...
		await pb.collection('posts').update(post.id, { caption: state.caption, hashtags, edited: true });
		const idx = posts.findIndex((p) => p.id === post.id);
		if (idx >= 0) posts[idx] = { ...posts[idx], caption: state.caption, hashtags, edited: true };
	...
	<main class="max-w-2xl mx-auto px-6 py-12">
		...
		{:else if business}
			...
			{#if user?.subscription_status === 'trial' && generationsLeft !== null}
				...
				>
					<p class="text-sm" style="color: var(--text-secondary); font-family: var(--font-primary)">
						{generationsLeft > 0
					...
					{#if generationsLeft === 0}
						<button
							...
							style="background: var(--coral); color: #fff; font-family: var(--font-primary); opacity: {subscribing ? '0.6' : '1'}; cursor: {subscribing ? 'not-allowed' : 'pointer'}"
						>
			...
			{#if user?.subscription_status === 'past_due'}
				...
				>
					<p class="text-sm" style="color: #856404; font-family: var(--font-primary)">
			...
			<div class="mb-10">
				{#if trialExhausted}
					<div
						...
						style="background: var(--surface); border: 1px solid var(--border)"
					>
						<p class="text-sm mb-3" style="color: var(--text); font-family: var(--font-primary)">
							Período de teste encerrado. Assine para continuar gerando conteúdo.
						</p>
						<button
							onclick={subscribe}
							disabled={subscribing}
							...
							style="background: var(--coral); color: #fff; font-family: var(--font-primary); opacity: {subscribing ? '0.6' : '1'}; cursor: {subscribing ? 'not-allowed' : 'pointer'}"
						>
							{subscribing ? 'Aguarde...' : 'Assinar — R$ 49,90/mês'}
						</button>
					</div>
				{:else}
					<button
						...
						disabled={generating}
						class="px-5 py-2.5 rounded-full text-sm font-medium transition-opacity"
						style="background: var(--coral); color: #fff; font-family: var(--font-primary); opacity: {generating ? '0.6' : '1'}; cursor: {generating ? 'not-allowed' : 'pointer'}"
					>
							...
							<div class="space-y-4">
								{#each batch.posts as post (post.id)}
									...
									>
										...
										{:else}
											<p class="text-sm leading-relaxed mb-3" style="color: var(--text)">
												{post.caption}
											</p>

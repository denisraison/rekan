<script lang="ts">
  ...
  // Nudge / engagement
  let clientFilter = $state<'todos' | 'inativos' | 'com_mensagens' | 'sazonal' | 'cobranca'>(
    'todos',
  );
  let nudgeText = $state('');
  ...
  let clientHealth = $derived.by(() => {
    ...
    for (const client of clients) {
      ...
      const lastMsg =
        clientMsgs.length > 0 ? clientMsgs.reduce((a, b) => (a.created > b.created ? a : b)) : null;
    ...
    switch (clientFilter) {
      ...
      case 'sazonal':
        ...
        return sortedClients.filter(
          (c) =>
            globalNearestSeasonal!.niches.length === 0 ||
            globalNearestSeasonal!.niches.includes(c.type),
        );
      case 'cobranca':
  ...
  let nudgeTier = $derived.by(() => {
    ...
    return (
      NUDGE_TEMPLATES.find((t) => h.daysSinceMsg >= t.minDays && h.daysSinceMsg <= t.maxDays) ??
  ...
  onMount(async () => {
    ...
    const clientsRes = await pb
      .collection('businesses')
      .getList<Business>(1, 200, { sort: 'name' });
    ...
    unsubscribeBusinesses = await pb.collection('businesses').subscribe<Business>('*', (e) => {
      if (e.action === 'create') {
        if (!clients.some((c) => c.id === e.record.id)) {
          clients = [...clients, e.record].sort((a, b) => a.name.localeCompare(b.name));
        }
      } else if (e.action === 'update') {
    ...
    unsubscribePosts = await pb.collection('posts').subscribe<Post>('*', (e) => {
      if (e.action === 'create') {
  ...
  function selectClient(id: string) {
    ...
    if (client && health && health.daysSinceMsg >= 5 && health.daysSinceMsg !== 999) {
      ...
      nudgeText = tier.template.replace(
        '{name}',
        client.client_name ? client.client_name.split(' ')[0] : client.name,
      );
    } else {
      nudgeText = '';
    }
  ...
  async function startVoiceRecording() {
    ...
    try {
      ...
      if (name === 'NotAllowedError' || name === 'PermissionDeniedError') {
        voiceError =
          'Permissão do microfone negada. Permita o acesso nas configurações do navegador ou preencha os campos manualmente.';
      } else if (!navigator.mediaDevices) {
        voiceError =
          'O microfone requer uma conexão segura (HTTPS). Preencha os campos manualmente.';
      } else {
  ...
  async function cancelSubscription() {
    if (!selected) return;
    if (!confirm(`Cancelar assinatura de ${selected.name}? Essa ação não pode ser desfeita.`))
      return;
    cancelling = true;
    try {
  ...
  async function generate() {
    ...
    try {
      ...
      if (selectedMessages.size === 1 && !hasText && !hasAttachment) {
        ...
        const selectedContent = buildSelectedContent();
        if (selectedContent) parts.push(selectedContent);
  ...
  async function generateIdeas() {
    ...
    ideaError = '';
    try {
      const res = await pb.send(`/api/businesses/${selectedId}/posts:generateIdeas`, {
        method: 'POST',
        body: JSON.stringify({}),
      });
...
<div class="h-dvh flex flex-col" style="background: var(--bg)">
  <header
    class="border-b px-5 py-4 flex items-center gap-3 shrink-0 {mobileView !== 'list'
      ? 'hidden md:flex'
      : ''}"
  ...
  {:else}
    ...
    <main class="flex-1 flex flex-col md:flex-row overflow-hidden">
      ...
      <div
        class="w-full md:w-80 md:border-r flex flex-col flex-1 min-h-0 md:flex-none {mobileView ===
        'list'
          ? ''
          : 'hidden md:flex'}"
      ...
      >
                ...
                >
                  ...
                  <div class="flex gap-2 mt-2">
                    <button
                      ...
                      style="background: #25D366; color: #fff; opacity: {approvingId === msg.id
                        ? '0.6'
                        : '1'}"
                    >
                    ...
                    <button
                      ...
                      style="border-color: var(--border-strong); color: var(--text-secondary); opacity: {dismissingId ===
                      msg.id
                        ? '0.6'
                        : '1'}"
                    >
        ...
        {:else}
          ...
          {#if unreadClientsCount > 0 || inactiveCount > 0 || globalNearestSeasonal || pendingPaymentCount > 0 || scheduledMessageCount > 0}
            <div style="border-bottom: 1px solid var(--border); background: var(--bg);">
              {#if unreadClientsCount > 0}
                ...
                >
                  <span
                    style="width: 8px; height: 8px; border-radius: 9999px; background: var(--coral); flex-shrink: 0; display: inline-block;"
                  ></span>
                  {unreadClientsCount}
                  {unreadClientsCount === 1 ? 'cliente' : 'clientes'} com mensagens novas
                  <span
                    style="margin-left: auto; color: var(--text-muted); font-size: 20px; line-height: 1;"
                    >›</span
                  >
                </button>
              {/if}
              {#if inactiveCount > 0}
                ...
                >
                  <span
                    style="width: 8px; height: 8px; border-radius: 9999px; background: #EF4444; flex-shrink: 0; display: inline-block;"
                  ></span>
                  {inactiveCount}
                  {inactiveCount === 1 ? 'cliente' : 'clientes'} inativos
                  <span
                    style="margin-left: auto; color: var(--text-muted); font-size: 20px; line-height: 1;"
                    >›</span
                  >
              ...
              {#if pendingPaymentCount > 0}
                <button
                  onclick={() => {
                    clientFilter = 'cobranca';
                  }}
                  style="display: flex; align-items: center; gap: 10px; width: 100%; min-height: 48px; padding: 0 20px; text-align: left; font-size: 14px; color: var(--text-secondary); border-bottom: 1px solid var(--border); background: none;"
                >
                  <span
                    style="width: 8px; height: 8px; border-radius: 9999px; background: #F59E0B; flex-shrink: 0; display: inline-block;"
                  ></span>
                  {pendingPaymentCount}
                  {pendingPaymentCount === 1 ? 'cliente' : 'clientes'} com pagamento pendente
                  <span
                    style="margin-left: auto; color: var(--text-muted); font-size: 20px; line-height: 1;"
                    >›</span
                  >
              ...
              {#if scheduledMessageCount > 0}
                ...
                >
                  ...
                  {scheduledMessageCount === 1 ? 'mensagem sazonal' : 'mensagens sazonais'} para aprovar
                  <span
                    style="margin-left: auto; color: var(--coral); font-size: 20px; line-height: 1;"
                    >›</span
                  >
                </button>
              {/if}
              {#if globalNearestSeasonal}
                ...
                >
                  <span
                    style="width: 8px; height: 8px; border-radius: 9999px; background: var(--sage); flex-shrink: 0; display: inline-block;"
                  ></span>
                  {globalNearestSeasonal.label} em {globalNearestSeasonal.daysUntil}d ({globalNearestSeasonal.eligibleCount}
                  clientes)
                  <span
                    style="margin-left: auto; color: var(--text-muted); font-size: 20px; line-height: 1;"
                    >›</span
          ...
          <!-- Color legend -->
          <div
            style="display: flex; gap: 16px; align-items: center; padding: 8px 20px; background: var(--bg); border-bottom: 1px solid var(--border);"
          >
            <span style="font-size: 12px; color: var(--text-muted);">Estado:</span>
            <span
              style="display: flex; align-items: center; gap: 5px; font-size: 12px; color: var(--text-muted);"
            >
              <span
                style="width: 8px; height: 8px; border-radius: 9999px; background: #10B981; display: inline-block;"
              ></span>Ativo
            </span>
            <span
              style="display: flex; align-items: center; gap: 5px; font-size: 12px; color: var(--text-muted);"
            >
              <span
                style="width: 8px; height: 8px; border-radius: 9999px; background: #F59E0B; display: inline-block;"
              ></span>5–9d
            </span>
            <span
              style="display: flex; align-items: center; gap: 5px; font-size: 12px; color: var(--text-muted);"
            >
              <span
                style="width: 8px; height: 8px; border-radius: 9999px; background: #EF4444; display: inline-block;"
              ></span>+10d
            </span>
          </div>
            ...
            {:else}
              {#each filteredClients as client (client.id)}
                ...
                <button
                  ...
                  class="w-full text-left px-5 py-4 border-b transition-colors {selectedId ===
                  client.id
                    ? 'bg-(--coral-pale)'
                    : 'hover:bg-(--coral-pale)/40'}"
                ...
                >
                    ...
                    <div class="flex items-center gap-2.5 min-w-0">
                      ...
                      {#if client.invite_status && client.invite_status !== 'draft'}
                        ...
                        {#if client.invite_status === 'accepted' && client.invite_sent_at && Date.now() - new Date(client.invite_sent_at).getTime() > 48 * 3600000}
                          <span class="text-sm shrink-0" title="Aceito há mais de 48h sem pagamento"
                            >&#9888;</span
                          >
                        {/if}
                      {/if}
                  ...
                  <div class="flex items-center justify-between mt-1 pl-5 gap-2">
                    <span class="text-sm truncate" style="color: var(--text-muted); min-width: 0;"
                      >{client.type} · {client.city}</span
                    >
                    ...
                    {:else if health && health.daysSinceMsg < 999}
                      <span
                        ...
                        style="color: {health.daysSinceMsg >= 5
                          ? health.color
                          : 'var(--text-muted)'}"
                      >
                  ...
                  {#if client.charge_pending}
                    <div class="mt-1 pl-5">
                      <span
                        style="font-size: 13px; padding: 2px 10px; border-radius: 9999px; background: #FEF3C7; color: #92400E;"
      ...
      <div
        class="flex-1 flex flex-col overflow-hidden {mobileView === 'detail' ||
        mobileView === 'info'
          ? ''
          : 'hidden md:flex'}"
      >
          ...
          <div class="flex-1 overflow-y-auto p-5 md:p-6">
            ...
            >
              ...
              {#if voiceMode === 'done'}
                ...
                <div
                  class="flex items-center gap-3 mb-4 p-3 rounded-xl"
                  style="background: var(--bg); border: 1px solid var(--border)"
                >
                  <div class="text-sm" style="flex: 1; min-width: 0;">
                    <span class="font-semibold" style="color: var(--text)"
                      >{formName || 'Novo cliente'}</span
                    >
                    {#if formType || formCity}
                      <span style="color: var(--text-muted)">
                        · {[formType, formCity].filter(Boolean).join(', ')}</span
                      >
                    {/if}
                  </div>
                  <button
                    onclick={() => (voiceMode = 'idle')}
                ...
                <div
                  class="flex items-center gap-3 mb-5 p-3 rounded-xl"
                  style="background: var(--sage-pale); border: 1.5px solid var(--sage-light)"
                >
                  <div
                    class="flex items-center justify-center flex-shrink-0"
                    style="width: 32px; height: 32px; border-radius: 50%; background: var(--sage);"
                  >
                    <svg
                      width="16"
                      height="16"
                  ...
                  <div>
                    <p class="text-sm font-bold" style="color: var(--text); margin: 0 0 2px;">
                      Perfil extraído da gravação
                    </p>
                ...
                <div class="flex flex-col gap-4">
                      ...
                      {#each formServices as service, i}
                        <div class="flex gap-2 items-center">
                          <input
                            ...
                            style="min-height: 52px; border-color: {aiFilledFields.has('services')
                              ? 'var(--sage)'
                              : 'var(--border-strong)'}; background: {aiFilledFields.has('services')
                              ? 'var(--sage-pale)'
                          ...
                          <div class="relative w-28">
                            ...
                            <input
                              ...
                              style="min-height: 52px; border-color: {aiFilledFields.has('services')
                                ? 'var(--sage)'
                                : 'var(--border-strong)'}; background: {aiFilledFields.has(
                                'services',
                              )
                                ? 'var(--sage-pale)'
                                : 'var(--surface)'}; color: var(--text)"
                          ...
                          <button
                            ...
                            style="width: 40px; height: 52px; border: none; background: none; color: var(--text-muted); font-size: 22px; cursor: pointer; display: flex; align-items: center; justify-content: center; flex-shrink: 0;"
                            >×</button
                          >
                        </div>
                      {/each}
                  ...
                  <label class="flex flex-col gap-1.5">
                    <span class="text-base font-medium" style="color: var(--text)"
                      >Quem são os clientes?</span
                    >
                    <span class="text-sm" style="color: var(--text-muted)"
                      >ex: mulheres de 25 a 50 anos que moram no bairro</span
                    >
                    <textarea
                      bind:value={formTargetAudience}
                      placeholder="Descreve quem costuma ir lá..."
                      rows={2}
                      class="px-3 py-3 rounded-xl text-base outline-none border resize-none"
                      style="min-height: 52px; border-color: {aiFilledFields.has('target_audience')
                        ? 'var(--sage)'
                        : 'var(--border-strong)'}; background: {aiFilledFields.has(
                        'target_audience',
                      )
                  ...
                  <label class="flex flex-col gap-1.5">
                    <span class="text-base font-medium" style="color: var(--text)"
                      >Como é o ambiente?</span
                    >
                    <span class="text-sm" style="color: var(--text-muted)"
                      >ex: acolhedor, descontraído, serve cafezinho</span
                    >
                    <textarea
                      bind:value={formBrandVibe}
                      placeholder="Conta um pouco sobre o clima do lugar..."
                      rows={2}
                      class="px-3 py-3 rounded-xl text-base outline-none border resize-none"
                      style="min-height: 52px; border-color: {aiFilledFields.has('brand_vibe')
                        ? 'var(--sage)'
                        : 'var(--border-strong)'}; background: {aiFilledFields.has('brand_vibe')
                        ? 'var(--sage-pale)'
                        : 'var(--surface)'}; color: var(--text)"
                    ></textarea>
                  </label>
                  <label class="flex flex-col gap-1.5">
                    <span class="text-base font-medium" style="color: var(--text)"
                      >O que faz diferente?</span
                    >
                    <span class="text-sm" style="color: var(--text-muted)"
                      >ex: agenda lotada às quintas</span
                    >
                    <textarea
                      bind:value={formQuirks}
                      placeholder="Detalhes que fazem a cliente escolher esse lugar..."
                      rows={3}
                      class="px-3 py-3 rounded-xl text-base outline-none border resize-none"
                      style="border-color: {aiFilledFields.has('quirks')
                        ? 'var(--sage)'
                        : 'var(--border-strong)'}; background: {aiFilledFields.has('quirks')
                        ? 'var(--sage-pale)'
                        : 'var(--surface)'}; color: var(--text)"
                    ></textarea>
                  </label>
                ...
                <div class="text-center mt-4">
                  <button
                    onclick={resetVoice}
                    class="text-sm inline-flex items-center gap-1.5 p-3"
                ...
                <div class="flex flex-col md:flex-row gap-3 mt-4">
                  ...
                  <button
                    onclick={saveClient}
                    disabled={formSaving}
                    class="px-5 py-3 rounded-full text-base font-medium"
              ...
              {:else}
                ...
                {#if voiceMode === 'manual'}
                  <button
                    onclick={resetVoice}
                    class="w-full flex items-center gap-3 mb-5"
                    style="min-height: 64px; background: var(--coral-pale); border: none; border-bottom: 1.5px solid var(--coral-light); border-radius: 12px; padding: 14px 16px; cursor: pointer; text-align: left;"
                  >
                    <div
                      class="flex items-center justify-center flex-shrink-0"
                      style="width: 36px; height: 36px; border-radius: 50%; background: var(--coral);"
                    >
                    ...
                    <p
                      ...
                      style="color: var(--text-secondary); margin: 0; line-height: 1.4;"
                    >
                      Prefere gravar uma descrição? É mais rápido.
                    </p>
                    <span
                      class="text-sm font-bold"
                      style="color: var(--coral); white-space: nowrap;">Gravar →</span
                    >
                  </button>
                {/if}
                ...
                <div class="flex flex-col gap-4">
                  ...
                  <div class="flex gap-3">
                    ...
                    <label class="flex flex-col gap-1.5 w-28">
                      ...
                      >
                        ...
                        {#each STATES as stateCode}<option value={stateCode}>{stateCode}</option
                          >{/each}
                      </select>
                    </label>
                  </div>
                  ...
                  {#if voiceMode === 'idle'}
                    ...
                    <div
                      ...
                      style="background: var(--coral-pale); border: 1.5px solid var(--coral-light)"
                    >
                      <button
                        onclick={startVoiceRecording}
                        aria-label="Gravar descrição"
                        style="width: 72px; height: 72px; border-radius: 50%; background: var(--coral); border: none; display: flex; align-items: center; justify-content: center; cursor: pointer; flex-shrink: 0; box-shadow: 0 4px 16px rgba(249,115,104,0.35);"
                      >
                      ...
                      <div>
                        <p class="text-base font-bold" style="margin: 0 0 4px; color: var(--text)">
                          Gravar descrição
                        </p>
                        <p
                          class="text-sm"
                          style="margin: 0; color: var(--text-secondary); line-height: 1.4;"
                        >
                          Toca no microfone e fala sobre a cliente
                        </p>
                      </div>
                    </div>
                    <div class="text-center">
                      <button
                        onclick={() => (voiceMode = 'manual')}
                        class="text-sm p-3"
                        style="color: var(--text-muted); border: none; background: none; cursor: pointer; text-decoration: underline; text-underline-offset: 3px;"
                    ...
                    >
                      <div class="flex items-center" style="height: 72px;">
                        <button
                          ...
                          style="width: 72px; height: 100%; background: #FEF2F2; border: none; border-right: 1px solid var(--border-strong); display: flex; align-items: center; justify-content: center; cursor: pointer; flex-shrink: 0;"
                        >
                          <svg
                            width="20"
                            height="20"
                        ...
                        <div class="flex-1 flex items-center justify-center gap-3">
                          <div
                            style="width: 10px; height: 10px; border-radius: 50%; background: #EF4444; flex-shrink: 0; animation: blink 1s ease-in-out infinite;"
                          ></div>
                          <span
                            style="font-size: 26px; font-weight: 700; letter-spacing: 0.04em; color: var(--text); font-variant-numeric: tabular-nums;"
                            >{fmtTime(recordingSeconds)}</span
                          >
                        ...
                        <button
                          ...
                          style="width: 72px; height: 100%; background: var(--coral); border: none; border-left: 1px solid var(--coral-light); display: flex; align-items: center; justify-content: center; cursor: pointer; flex-shrink: 0;"
                        >
                          <svg
                            width="22"
                            height="22"
                  ...
                  {:else if voiceMode === 'analyzing'}
                    <!-- Analyzing bar -->
                    <div
                      class="rounded-2xl flex items-center justify-center gap-3"
                      style="height: 72px; border: 1.5px solid var(--border-strong);"
                    >
                      <div
                        style="width: 22px; height: 22px; border-radius: 50%; border: 2.5px solid var(--coral); border-top-color: transparent; animation: spin 0.8s linear infinite; flex-shrink: 0;"
                      ></div>
                      <span class="text-base font-medium" style="color: var(--text-secondary)"
                        >Analisando...</span
                      >
                    </div>
                  {:else}
                    ...
                    <div>
                      <span class="text-base font-medium" style="color: var(--text)">Serviços</span>
                      <p class="text-sm mt-0.5 mb-1.5" style="color: var(--text-muted)">
                        Coloca os serviços mais pedidos e o preço de cada um.
                      </p>
                        ...
                        {#each formServices as service, i}
                          <div class="flex gap-2 items-center">
                            <input
                              ...
                              placeholder="Nome do serviço"
                              class="flex-1 px-3 py-3 rounded-xl text-base outline-none border"
                              style="min-height: 52px; border-color: var(--border-strong); background: var(--surface); color: var(--text)"
                            />
                            <div class="relative w-28">
                              <span
                                class="absolute left-3 top-1/2 -translate-y-1/2 text-base"
                ...
                {#if inviteUrl}
                  ...
                  >
                    <p class="text-base font-medium mb-2" style="color: var(--text)">
                      Convite enviado!
                    </p>
                  ...
                  <div class="flex flex-col md:flex-row gap-3 mt-6">
                    ...
                    {#if voiceMode === 'idle'}
                      <button
                        disabled
                        class="px-5 py-3 rounded-full text-base font-medium"
                        style="background: rgba(17,17,22,0.08); color: var(--text-muted); cursor: not-allowed;"
                    ...
                    {:else}
                      <button
                        ...
                        style="background: var(--coral); color: #fff; opacity: {formSaving
                          ? '0.6'
                          : '1'}; cursor: {formSaving ? 'not-allowed' : 'pointer'}"
                        >{formSaving ? 'Salvando...' : 'Salvar'}</button
                      >
                      <button
                        onclick={saveAndInvite}
                        disabled={formSaving}
                        ...
                        style="background: #25D366; color: #fff; opacity: {formSaving
                          ? '0.6'
                          : '1'}; cursor: {formSaving ? 'not-allowed' : 'pointer'}"
                        >{formSaving ? 'Salvando...' : 'Salvar e Enviar Convite'}</button
                      >
                    {/if}
                  </div>
                {/if}
        ...
        {:else if selected}
          ...
          {#if mobileView === 'info'}
            <div class="flex-1 flex flex-col overflow-hidden">
              ...
              <div
                style="background: var(--surface); border-bottom: 1px solid var(--border); display: flex; align-items: center; min-height: 60px; padding: 0 16px; gap: 4px; flex-shrink: 0;"
              >
                <button
                  onclick={() => {
                    mobileView = 'detail';
                ...
                <!-- Avatar -->
                <div
                  style="width: 44px; height: 44px; border-radius: 9999px; overflow: hidden; flex-shrink: 0; display: flex; align-items: center; justify-content: center; font-size: 16px; font-weight: 600; background: var(--coral-pale); color: var(--coral);"
                >
                  {#if profilePictureUrl(selected)}
                    <img
                      src={profilePictureUrl(selected)!}
                      alt={selected.name}
                ...
                <div style="flex: 1; min-width: 0;">
                  <div
                    style="font-size: 17px; font-weight: 600; color: var(--text); overflow: hidden; text-overflow: ellipsis; white-space: nowrap;"
                  >
                    {selected.name}
                  </div>
              ...
              <div class="flex-1 overflow-y-auto">
                <!-- Status strip -->
                <div
                  style="background: var(--surface); padding: 14px 20px; border-bottom: 1px solid var(--border); display: flex; align-items: center; gap: 10px; flex-wrap: wrap;"
                >
                  {#if selected.tier}
                    <span
                      style="font-size: 14px; padding: 4px 12px; border-radius: 9999px; background: var(--bg); color: var(--text-secondary); border: 1px solid var(--border-strong); font-weight: 500;"
                      >{selected.tier}</span
                    >
                  {/if}
                  {#if selected.invite_status && selected.invite_status !== 'draft'}
                    <span
                      style="font-size: 14px; padding: 4px 12px; border-radius: 9999px; font-weight: 500; {selected.invite_status ===
                      'invited'
                        ? 'background: #FEF3C7; color: #92400E'
                        : selected.invite_status === 'accepted'
                          ? 'background: #DBEAFE; color: #1E40AF'
                          : selected.invite_status === 'active'
                            ? 'background: #DEF7EC; color: #03543F'
                            : selected.invite_status === 'payment_failed'
                              ? 'background: #FEE2E2; color: #991B1B'
                              : selected.invite_status === 'cancelled'
                                ? 'background: #FEE2E2; color: #991B1B; text-decoration: line-through'
                                : 'background: var(--border); color: var(--text-muted)'}"
                    >
                      {selected.invite_status === 'invited'
                        ? 'convite'
                        : selected.invite_status === 'accepted'
                          ? 'aceito'
                          : selected.invite_status === 'active'
                  ...
                  {#if selected.invite_status === 'active' && selected.next_charge_date}
                    <span style="font-size: 14px; color: var(--text-muted); margin-left: 4px;">
                      Próx. cobrança: {new Date(selected.next_charge_date).toLocaleDateString(
                        'pt-BR',
                  ...
                  {#if selected.charge_pending}
                    <span
                      style="font-size: 14px; padding: 4px 12px; border-radius: 9999px; font-weight: 500; background: #FEE2E2; color: #991B1B;"
                      >Pagamento pendente</span
                    >
                  {/if}
                ...
                {#if selected.services?.length > 0}
                  <span
                    style="font-size: 11px; font-weight: 700; letter-spacing: 0.08em; text-transform: uppercase; color: var(--text-muted); padding: 14px 20px 8px; background: var(--bg); border-top: 1px solid var(--border); display: block;"
                    >Serviços</span
                  >
                  <div style="background: var(--surface); padding: 4px 0 12px;">
                    {#each selected.services as svc}
                      <div
                        style="display: flex; align-items: baseline; gap: 12px; padding: 6px 20px;"
                      >
                        <span
                          style="font-size: 13px; font-weight: 600; color: var(--text-muted); white-space: nowrap; flex-shrink: 0; width: 100px; overflow: hidden; text-overflow: ellipsis;"
                          >{svc.name}</span
                        >
                        <span style="font-size: 15px; color: var(--text-secondary); flex: 1;"
                          >{svc.price_brl != null
                            ? `R$${svc.price_brl.toFixed(2).replace('.', ',')}`
                            : '—'}</span
                        >
                      </div>
                    {/each}
                ...
                {#if selected.target_audience || selected.brand_vibe || selected.quirks}
                  ...
                  <div style="background: var(--surface); padding: 4px 0 12px;">
                    {#if selected.target_audience}
                      ...
                      >
                        <span
                          style="font-size: 13px; font-weight: 600; color: var(--text-muted); white-space: nowrap; flex-shrink: 0; width: 64px;"
                          >Público</span
                        >
                        <span style="font-size: 15px; color: var(--text-secondary); flex: 1;"
                          >{selected.target_audience}</span
                        >
                      </div>
                    {/if}
                    {#if selected.brand_vibe}
                      ...
                      >
                        ...
                        >
                        <span
                          style="font-size: 15px; color: var(--text-secondary); flex: 1; font-style: italic;"
                          >{selected.brand_vibe}</span
                        >
                      </div>
                    {/if}
                    {#if selected.quirks}
                      ...
                      >
                        ...
                        >
                        <span
                          style="font-size: 15px; color: var(--text-secondary); flex: 1; white-space: pre-wrap;"
                          >{selected.quirks}</span
                        >
                      </div>
                    {/if}
                  </div>
                ...
                {#if suggestions.length > 0}
                  ...
                  >
                    <div style="display: flex; align-items: center; gap: 8px;">
                      <span
                        style="font-size: 11px; font-weight: 700; letter-spacing: 0.08em; text-transform: uppercase; color: var(--sage-dark);"
                        >Sugestões de perfil</span
                      >
                      <span
                        style="font-size: 12px; font-weight: 700; padding: 1px 7px; border-radius: 9999px; background: var(--sage); color: #fff;"
                        >{suggestions.length}</span
                      >
                    </div>
                    <span style="font-size: 13px; color: var(--text-muted);"
                      >{suggestionsOpen ? '▴' : '▾'}</span
                    >
                  </button>
                  {#if suggestionsOpen}
                    <div style="background: var(--surface);">
                      {#each suggestions as sug (sug.id)}
                        <div style="padding: 12px 20px; border-bottom: 1px solid var(--border);">
                          <span
                            style="font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.06em; color: var(--text-muted);"
                          >
                            {sug.field === 'services'
                              ? 'Serviço detectado'
                              : sug.field === 'quirks'
                                ? 'Diferencial detectado'
                                : sug.field === 'target_audience'
                                  ? 'Público detectado'
                                  : 'Estilo detectado'}
                          </span>
                          <p
                            style="font-size: 15px; color: var(--text-secondary); margin: 4px 0 10px; line-height: 1.5;"
                          >
                            {sug.suggestion}
                          </p>
                ...
                {#if clientPosts.length > 0}
                  ...
                  <div style="background: var(--surface);">
                    {#each clientPosts.slice(0, historyLimit) as post (post.id)}
                      ...
                      <div
                        style="padding: 12px 20px; border-bottom: 1px solid var(--border); display: flex; gap: 12px; align-items: flex-start;"
                      >
                        <span
                          style="font-size: 14px; color: var(--text-muted); white-space: nowrap; padding-top: 2px; width: 48px; flex-shrink: 0;"
                        >
                        ...
                        <div style="flex: 1; min-width: 0;">
                          <button
                            onclick={() => {
                              ...
                              if (postExpanded) next.delete(post.id);
                              else next.add(post.id);
                              expandedPosts = next;
                            }}
                            style="font-size: 15px; color: var(--text-secondary); line-height: 1.5; text-align: left; width: 100%; background: none; border: none; padding: 0; {postExpanded
                              ? ''
                          ...
                          {#if !postExpanded}
                            <span
                              style="font-size: 13px; color: var(--text-muted); margin-top: 2px; display: block;"
                              >toque para expandir</span
                            >
                          {/if}
                          {#if postExpanded && post.production_note}
                            <p
                              style="font-size: 14px; font-style: italic; margin-top: 4px; color: var(--text-muted);"
                            >
                    ...
                    {#if clientPosts.length > historyLimit}
                      <button
                        onclick={() => {
                          historyLimit = clientPosts.length;
                        }}
                ...
                {#if nudgeTier || nudgeText}
                  <span
                    style="font-size: 11px; font-weight: 700; letter-spacing: 0.08em; text-transform: uppercase; padding: 14px 20px 8px; display: block; {nudgeTier
                      ? 'background: #FFF7ED; border-top: 1px solid #FDE68A; color: #92400E;'
                      : 'background: var(--bg); border-top: 1px solid var(--border); color: var(--text-muted);'}"
                  >
                    {nudgeTier
                  ...
                  <div
                    style="padding: 12px 20px 16px; border-bottom: 1px solid var(--border); {nudgeTier
                      ? 'background: #FFFBEB;'
                  ...
                  >
                    ...
                    <div class="flex items-center gap-2 mt-2">
                      <button
                        ...
                        style="min-height: 48px; padding: 0 24px; border-radius: 9999px; font-size: 15px; font-weight: 600; background: #25D366; color: #fff; opacity: {sendingNudge ||
                        !nudgeText.trim() ||
                        blockReason
                          ? '0.6'
                          : '1'}; cursor: {sendingNudge || !nudgeText.trim() || blockReason
                          ? 'not-allowed'
                          : 'pointer'};"
                      >
                ...
                {#if upcomingDates.length > 0}
                  ...
                  >
                  <div
                    style="background: var(--surface); padding: 8px 20px 16px; display: flex; flex-wrap: wrap; gap: 8px;"
                  >
                    {#each upcomingDates as sd}
                      <button
                        onclick={() => {
                          prefillSeasonalMessage(sd.template);
                        }}
                ...
                {#if selected.invite_status === 'active'}
                  <div
                    style="margin: 16px 20px 24px; padding: 16px; border-radius: 16px; border: 1px solid #FCA5A5; background: #FFF5F5;"
                  >
                    <p
                      style="font-size: 14px; color: #991B1B; margin-bottom: 10px; line-height: 1.5;"
                    >
                    ...
                    <button
                      ...
                      disabled={cancelling}
                      style="min-height: 48px; padding: 0 20px; border-radius: 9999px; font-size: 14px; font-weight: 600; border: 1.5px solid #EF4444; color: #EF4444; background: transparent; opacity: {cancelling
                        ? '0.6'
                        : '1'};"
                    >
          ...
          >
            ...
            >
              <div class="flex items-center gap-2">
                ...
                >
                  <!-- Avatar -->
                  <div
                    class="shrink-0 w-9 h-9 rounded-full overflow-hidden flex items-center justify-center text-sm font-semibold"
            ...
            {#if generatingIdeas || ideaDrafts !== null}
              <div
                class="md:hidden absolute inset-0 flex flex-col z-10"
                style="background: var(--bg)"
              >
                <div
                  class="flex items-center gap-3 px-4 shrink-0"
                  style="min-height: 60px; background: var(--surface); border-bottom: 1px solid var(--border);"
                >
                  {#if !generatingIdeas}
                    <button
                      onclick={() => {
                ...
                {#if generatingIdeas}
                  <div class="flex-1 flex flex-col items-center justify-center gap-4 p-8">
                    ...
                    <p class="text-base text-center" style="color: var(--text-muted)">
                      Escrevendo 3 ideias<br />caprichadas pra você...
                    </p>
                  </div>
                {:else}
                  <div class="flex-1 overflow-y-auto p-4 flex flex-col gap-4">
                    {#each ideaDrafts! as draft, i}
                      <button
                        ...
                        style="background: var(--surface); border: 2px solid {selectedIdeas.has(i)
                          ? 'var(--coral)'
                          : 'var(--border)'};"
                      >
                        <div class="flex items-start gap-3">
                          <div
                            ...
                            style="border: 2px solid {selectedIdeas.has(i)
                              ? 'var(--coral)'
                              : 'var(--border)'}; background: {selectedIdeas.has(i)
                              ? 'var(--coral)'
                              : 'transparent'};"
                          >
                            {#if selectedIdeas.has(i)}
                          ...
                          <div class="min-w-0 flex-1">
                            <p
                              ...
                              style="color: var(--text); white-space: pre-wrap"
                            >
                              {draft.caption}
                            </p>
                    ...
                    >
                      ...
                      {:else}
                        <button
                          ...
                          class="flex-1 rounded-full text-base font-semibold"
                          style="min-height: 52px; background: #25D366; color: #fff; opacity: {sendingIdeas
                            ? '0.6'
                            : '1'}"
                          >{sendingIdeas
                            ? 'Enviando...'
                            : `Enviar ${selectedIdeas.size} selecionadas`}</button
                        >
                      {/if}
                      <button
                        onclick={() => {
            ...
            {#if result && showReviewOverlay}
              <div class="absolute inset-0 flex flex-col z-10" style="background: var(--bg)">
                ...
                <div class="flex-1 overflow-y-auto p-4 flex flex-col gap-4">
                  <div>
                    <div class="flex items-center justify-between mb-1">
                      ...
                      >
                      <button
                        onclick={() => copyWithFeedback('caption', editingCaption)}
                        class="text-sm py-1"
                        style="color: var(--coral)"
                      >
                  ...
                  <div>
                    <div class="flex items-center justify-between mb-1">
                      ...
                      >
                      <button
                        onclick={() => copyWithFeedback('hashtags', result!.hashtags.join(' '))}
                        class="text-sm py-1"
                        style="color: var(--coral)"
                      >
                  ...
                  {#if result.production_note}
                    <div>
                      <div class="flex items-center justify-between mb-1">
                        <span class="text-sm font-medium" style="color: var(--text-muted)"
                          >Nota de produção</span
                        >
                        <button
                          onclick={() => copyWithFeedback('note', result!.production_note!)}
                          class="text-sm py-1"
                          style="color: var(--coral)"
                        >
                ...
                <div
                  class="shrink-0 px-4 py-3 flex flex-col gap-2"
                  style="background: var(--surface); border-top: 1px solid var(--border);"
                >
                  {#if blockReason}
                    <span class="text-sm" style="color: var(--text-muted)"
              ...
              {:else}
                {#each groupedMessages as group}
                  <div class="flex items-center gap-3 my-1">
                    ...
                    <span class="text-sm shrink-0" style="color: var(--text-muted)"
                      >{group.label}</span
                    >
                    <hr class="flex-1" style="border-color: var(--border)" />
                  </div>
                  {#each group.msgs as msg (msg.id)}
                    ...
                    >
                      ...
                      <div
                        ...
                        style="max-width: 280px; background: {selectedMessages.has(msg.id)
                          ? 'var(--coral-pale)'
                          : msg.direction === 'outgoing'
                            ? 'var(--coral-pale)'
                            : 'var(--surface)'}; border: 1px solid {msg.direction === 'outgoing'
                          ? 'var(--coral-light)'
                          : 'var(--border)'}; color: var(--text); line-height: 1.5; {inputMode ===
                        'generate'
                          ? 'cursor: pointer;'
                        ...
                        onclick={() => {
                          if (inputMode === 'generate') toggleMessageSelection(msg.id);
                        }}
                        onkeydown={(e) => {
                          if (inputMode === 'generate' && (e.key === 'Enter' || e.key === ' ')) {
                            e.preventDefault();
            ...
            <div
              ...
              style="border-color: {inputMode === 'generate'
                ? 'var(--coral)'
                : 'var(--border)'}; background: var(--surface); {inputMode === 'generate'
                ...
                <div class="hidden md:flex flex-col gap-3 mb-2">
                  ...
                  {#each ideaDrafts as draft, i}
                    <button
                      ...
                      style="background: var(--bg); border: 2px solid {selectedIdeas.has(i)
                        ? 'var(--coral)'
                        : 'var(--border)'};"
                    >
                      <div class="flex items-start gap-3">
                    ...
                    <div class="flex gap-2">
                      {#if selectedIdeas.size === 1}
                        <button
                          ...
                          style="background: var(--coral); color: #fff">Revisar e enviar</button
                        >
                      {:else}
                        <button
                          onclick={sendSelectedIdeas}
                          disabled={sendingIdeas}
                          class="px-5 py-2.5 rounded-full text-sm font-medium"
                          style="background: #25D366; color: #fff; opacity: {sendingIdeas
                            ? '0.6'
                            : '1'}"
                          >{sendingIdeas
                            ? 'Enviando...'
                      ...
                      <button
                        ...
                        style="color: var(--destructive)">Limpar</button
                      >
                    </div>
                  {/if}
                </div>
              ...
              {#if !blockReason}
                  ...
                  {#if inputMode === 'generate' && ideaDrafts === null}
                    ...
                    {#if showGenerateIdeasButton}
                      <button
                        ...
                        class="text-sm px-3 py-1.5 rounded-full font-medium flex items-center gap-1.5"
                        style="background: var(--sage-pale); color: var(--text-secondary); opacity: {generatingIdeas ||
                        generating
                          ? '0.5'
                          : '1'}"
                      >
                  ...
                  >
                    ...
                    {:else}
                      <svg
                        ...
                        stroke-linejoin="round"
                        ><path
                          d="M12 3l1.09 3.26L16.36 7.5l-3.27 1.15L12 12l-1.09-3.35L7.64 7.5l3.27-1.24L12 3z"
                        /><path
                          d="M5 16l.55 1.64L7.18 18.2 5.55 18.76 5 20.4l-.55-1.64L2.82 18.2l1.63-.56L5 16z"
                        /><path
                          d="M18 12l.55 1.64 1.63.56-1.63.56L18 16.4l-.55-1.64-1.63-.56 1.63-.56L18 12z"
                        /></svg
                      >
                      Post
                    {/if}
                ...
                {#if attachedPreview}
                  <div class="flex items-center gap-2 px-1">
                    ...
                    <span class="text-xs truncate max-w-[200px]" style="color: var(--text-muted)"
                ...
                <div class="flex gap-2 items-center relative">
                  ...
                  {#if showAttachMenu}
                    ...
                    ></button>
                    <div
                      class="absolute bottom-12 left-0 z-20 rounded-xl shadow-lg border p-2 flex flex-col gap-1"
                      style="background: var(--bg); border-color: var(--border-strong); min-width: 180px;"
                    >
                      <button
                  ...
                  <input
                    ...
                    onkeydown={(e) => {
                      if (e.key === 'Enter' && !e.shiftKey) {
                        ...
                        else if (message.trim() || selectedMessages.size > 0 || attachedFile)
                          generate();
                      }
                    }}
                  />
                  {#if inputMode === 'generate'}
                    <button
                      ...
                      disabled={generating ||
                        generatingIdeas ||
                        (!message.trim() && selectedMessages.size === 0 && !attachedFile)}
                  ...
                  {:else}
                    <button
                      ...
                      style="background: #25D366; color: #fff; opacity: {sendingQuick ||
                      sendingMedia ||

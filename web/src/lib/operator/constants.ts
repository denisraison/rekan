...
export function findNudgeTier(daysSinceMsg: number): NudgeTemplate | null {
  ...
  return (
    NUDGE_TEMPLATES.find(
      (t) => daysSinceMsg >= t.minDays && daysSinceMsg <= t.maxDays,
    ) ?? NUDGE_TEMPLATES[NUDGE_TEMPLATES.length - 1]
  );
}

/** Resolve a nudge/seasonal template with the client's first name. */
export function resolveTemplate(template: string, clientName: string, businessName: string): string {
...
export function findNearestSeasonal(businessTypes: string[]): NearestSeasonal | null {
  ...
  for (const sd of SEASONAL_DATES_SORTED) {
    ...
    if (!date) continue;
    const eligibleCount = businessTypes.filter(t => sd.niches.length === 0 || sd.niches.includes(t)).length;
    if (eligibleCount > 0) {
      return { ...sd, daysUntil: Math.ceil((date.getTime() - now.getTime()) / 86400000), eligibleCount };
    }
  }
  return null;
}
